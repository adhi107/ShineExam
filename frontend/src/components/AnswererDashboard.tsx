...
const navItems: Array<{ view: PortalView; label: string; icon: string }> = [
  { view: "tests", label: "My Tests", icon: "▣" },
  { view: "report", label: "Reports", icon: "▤" },
  ...
  return (
    <div className="candidate-shell">
      <aside className="candidate-sidebar">
        <div className="candidate-logo"><ShineLogo /></div>
        <nav className="candidate-nav">
          <div className={`my-tests-nav ${view === "tests" ? "active" : ""}`}>
            <button className="my-tests-main" onClick={() => goTo("tests")}><span className="candidate-nav-icon">▣</span><span>My Tests</span></button>
            <button className={`my-tests-collapse ${testsTreeOpen ? "open" : ""}`} onClick={() => setTestsTreeOpen(open => !open)} aria-label={testsTreeOpen ? "Collapse My Tests" : "Expand My Tests"}>⌄</button>
          </div>
          {testsTreeOpen && <div className="exam-categories under-tests">
            {examGroups.map(group => <div className="exam-group" key={group.slug}>
              <button className={`exam-group-button ${expandedGroups.includes(group.slug) ? "open" : ""}`} onClick={() => toggleExpanded(group.slug, expandedGroups, setExpandedGroups)}><i />{group.label}<span>⌄</span></button>
              {expandedGroups.includes(group.slug) && <div className="exam-group-children">{group.exams.map(exam => <div className="exam-subgroup" key={exam.slug}>
                <button className={`exam-name-button ${expandedExams.includes(exam.slug) ? "open" : ""} ${selectedExamPage?.examSlug === exam.slug ? "selected" : ""}`} onClick={() => toggleExpanded(exam.slug, expandedExams, setExpandedExams)}><span>›</span><b>{exam.label}</b></button>
                {expandedExams.includes(exam.slug) && <div className="exam-stage-links">{examPages.filter(page => page.subcategoryId === exam.id).map(page => <button key={page.stageSlug} className={selectedExamPage?.subcategoryId === page.subcategoryId && selectedExamPage.stageSlug === page.stageSlug ? "active" : ""} onClick={() => openExamPage(page)}><i />{page.stageLabel}</button>)}</div>}
              </div>)}</div>}
            </div>)}
          </div>}
          {navItems.filter(item => item.view !== "tests").map(item => <button key={item.view} className={`candidate-nav-item ${view === item.view ? "active" : ""}`} onClick={() => goTo(item.view)}><span className="candidate-nav-icon">{item.icon}</span><span>{item.label}</span></button>)}
        </nav>
      ...
      <main className="candidate-main">
        <header className="candidate-topbar">
          <div><span className="mobile-brand">SHINE</span><h1>{view === "tests" ? (selectedExamPage ? `${selectedExamPage.examLabel} ${selectedExamPage.stageLabel}` : "My Tests") : view === "report" ? "Performance Report" : navItems.find(n => n.view === view)?.label}</h1></div>
          <div className="candidate-top-actions">
            ...
            <div className="notification-wrap">
              <button className="icon-button" aria-label="Notifications" onClick={() => { setNotificationsOpen(v => !v); setProfileOpen(false); }}>♢{unreadNotifications > 0 && <i />}</button>
              {notificationsOpen && <div className="notification-menu">
                <div className="notification-menu-head"><strong>Notifications ({unreadNotifications})</strong><button onClick={() => markNotificationRead("all")}>Mark all read</button></div>
                {notifications.length === 0 ? <div className="notification-empty">You are all caught up.</div> : notifications.map(item => <button className={item.read ? "read" : "unread"} key={item.id} onClick={() => { void markNotificationRead(item.id); setNotificationsOpen(false); goTo(item.target); }}><span>{item.type === "test" ? "NEW" : item.type === "document" ? "DOC" : item.type === "announcement" ? "NEWS" : "✓"}</span><div><strong>{item.title}</strong><small>{item.message}</small></div></button>)}
              </div>}
            </div>
            <div className="profile-wrap">
              <button className="profile-button" onClick={() => { setProfileOpen(v => !v); setNotificationsOpen(false); }}><span>{userName.charAt(0).toUpperCase()}</span><div><strong>{userName}</strong><small>Candidate</small></div><b>⌄</b></button>
        ...
        {view === "tests" && <section className="portal-page tests-page">
          ...
          <div className="page-toolbar"><div><div className="exam-breadcrumb"><button onClick={() => goTo("tests")}>My Tests</button>{selectedExamPage && <><span>›</span><span>{selectedExamPage.groupLabel}</span><span>›</span><span>{selectedExamPage.examLabel}</span><span>›</span><b>{selectedExamPage.stageLabel}</b></>}</div><h2>{selectedExamPage ? `${selectedExamPage.examLabel} ${selectedExamPage.stageLabel}` : `${testTab[0].toUpperCase()+testTab.slice(1)} Tests`}</h2><p>{testTab === "completed" ? "Review your completed assessments and reports" : "Assessments available for your account"}</p></div><div className="view-toggle"><button className={gridView ? "active" : ""} onClick={() => setGridView(true)}>▦</button><button className={!gridView ? "active" : ""} onClick={() => setGridView(false)}>☰</button></div></div>
          ...
          <div className={`portal-test-list ${gridView ? "grid" : "list"}`}>{visibleTests.map((test, index) => (
            <article className="portal-test-card" key={test.id}>
              ...
              <div className="test-card-heading"><span>{index % 2 ? "QUANTITATIVE" : "GENERAL"}</span><h3>{test.name}</h3></div>
              ...
              {testTab === "completed" ? <button className="shine-primary" onClick={() => { const r=history.find(h => h.examId === test.id) || history[0]; setSelectedReport(r || null); setReportTab("score"); goTo("report"); }}>View report <span>→</span></button> : testTab === "upcoming" ? <button className="shine-primary" disabled>Opens {validityDate(test.availableFrom)}</button> : testTab === "missed" ? <button className="shine-primary" disabled>Expired</button> : <button className="shine-primary" disabled={!!startingId} onClick={() => startExam(test.id)}>{startingId === test.id ? "Starting…" : "Start test"}<span>→</span></button>}
            </article>
          ))}</div>}
        </section>}

        {view === "report" && <section className="portal-page report-page">
          <div className="report-selector"><div><span>REPORT FOR</span><div className="report-value-help"><ValueHelpField label="Search Reports" placeholder="Search test, date or result" value={reportSearch} options={history.map(item=>({value:item.testName,label:item.testName,keywords:[formatDate(item.submittedAt),item.passed?"Passed":"Needs improvement"]}))} onChange={setReportSearch} allowFreeText compact/></div><select value={currentReport?.attemptId || ""} onChange={e => setSelectedReport(history.find(h => h.attemptId === e.target.value) || null)}>{filteredHistory.length ? filteredHistory.map(h => <option key={h.attemptId} value={h.attemptId}>{h.testName} — {formatDate(h.submittedAt)}</option>) : <option value="">No matching reports</option>}</select></div>{currentReport && <span className={`result-pill ${currentReport.passed ? "pass" : "fail"}`}>{currentReport.passed ? "Passed" : "Needs improvement"}</span>}</div>
          ...
          {!currentReport ? <div className="portal-empty"><span>▤</span><h3>No report available</h3><p>Complete a test and your detailed analytics will appear here.</p></div> : <>
            <div className="report-kpis"><div><span>Total Questions</span><strong>{questionCount}</strong></div><div><span>Total Marks</span><strong>{total}</strong></div><div><span>Time Spent</span><strong>{formatTime(currentReport.timeSpentSec)}</strong></div><div><span>Percentile</span><strong>{Math.min(99, Math.round(60 + pct * .35))}<small>th</small></strong></div></div>
            {reportTab === "score" && <ScoreReport score={score} total={total} pct={pct} correct={correct} incorrect={incorrect} unanswered={unanswered} />}
            {reportTab === "subject" && <SubjectReport score={score} total={total} pct={pct} time={formatTime(currentReport.timeSpentSec)} />}
...
const ScoreReport = ({score,total,pct,correct,incorrect,unanswered}:any) => <div className="report-grid"><article className="report-card score-summary"><h3>Performance Summary</h3><div className="score-ring" style={{'--score': `${pct * 3.6}deg`} as React.CSSProperties}><div><strong>{score}</strong><span>out of {total}</span></div></div><h2>{pct >= 70 ? "Excellent work!" : pct >= 40 ? "Good progress" : "Keep practicing"}</h2><p>You scored <b>{pct.toFixed(1)}%</b> in this assessment.</p><div className="score-legend"><span className="correct"><i />Correct <b>{correct}</b></span><span className="wrong"><i />Incorrect <b>{incorrect}</b></span><span><i />Unattempted <b>{unanswered}</b></span></div></article><article className="report-card"><h3>Score Breakdown</h3><div className="horizontal-metrics"><Metric label="Correct answers" value={correct} max={total} color="#27b487"/><Metric label="Incorrect answers" value={incorrect} max={total} color="#ef6a78"/><Metric label="Unattempted" value={unanswered} max={total} color="#a7b0c0"/></div><div className="rank-strip"><div><small>Your rank</small><strong>#{Math.max(1,Math.round(400-pct*3))}</strong></div><div><small>Accuracy</small><strong>{pct.toFixed(1)}%</strong></div><div><small>Average score</small><strong>{(total*.52).toFixed(1)}</strong></div></div></article></div>;
const Metric=({label,value,max,color}:any)=><div className="metric-row"><div><span>{label}</span><b>{value}/{max}</b></div><div className="metric-track"><i style={{width:`${max ? value/max*100 : 0}%`,background:color}} /></div></div>;
const SubjectReport=({score,total,pct,time}:any)=><div className="report-card table-card"><h3>Subject Performance</h3><div className="report-table"><div className="table-head"><span>Subject name</span><span>Attempts</span><span>Score</span><span>Accuracy</span><span>Time spent</span></div><div className="table-row"><strong>Quantitative & General Aptitude</strong><span>{total}</span><span>{score} / {total}</span><span>{pct.toFixed(1)}%</span><span>{time}</span></div></div><div className="subject-analysis"><div><span>Subject score</span><strong>{score}<small>/{total}</small></strong><Metric label="Mastery" value={pct} max={100} color="#2f6fed"/></div><div className="mini-pie" style={{'--score':`${pct*3.6}deg`} as React.CSSProperties}/></div></div>;
...
const SolutionReport=({testName,userName,review,onClose}:any)=>{
  ...
  const current=review[currentIndex];
  ...
  const openSection=(section:string)=>{const index=review.findIndex((item:any)=>item.section===section);if(index>=0)setCurrentIndex(index)};
  ...
  return <div className="solution-paper">
    <header className="solution-paper-header"><div><ShineLogo compact/><h2>{testName}</h2></div><div><span className="solution-avatar">{String(userName).charAt(0).toUpperCase()}</span><strong>{userName}</strong><button onClick={onClose}>Exit solution</button></div></header>
    <div className="solution-section-bar">{sections.map(section=><button key={section} className={current.section===section?"active":""} onClick={()=>openSection(section)}>{section}</button>)}</div>
    <div className="solution-question-meta"><strong>Question {currentIndex+1}</strong><div><span className="difficulty-pill">Practice</span><span className={status(current)}>{status(current)==="correct"?"Correct":status(current)==="wrong"?"Incorrect":"Unattempted"}</span></div></div>
    <main className="solution-paper-body">
      <section className="solution-question-pane">
        <div className="solution-question-text">{current.question||`Question ${currentIndex+1}`}</div>
        <div className="solution-options">{(current.options||[]).map((option:string,index:number)=>{const correct=matches(current.correctAnswer,option,index);const chosen=matches(current.userAnswer,option,index);return <div key={`${option}-${index}`} className={`solution-option ${correct?"correct-option":""} ${chosen&&!correct?"wrong-option":""}`}><span>{String.fromCharCode(65+index)}</span><p>{option}</p>{correct&&<b>✓ Correct answer</b>}{chosen&&correct&&<em>Your answer</em>}{chosen&&!correct&&<em>✕ Your answer</em>}</div>})}</div>
        {(!current.options||current.options.length===0)&&<div className="solution-no-options">Answer review is unavailable for this legacy question.</div>}
        <div className="solution-explanation"><strong>Solution</strong><p>{current.isCorrect?"You selected the correct answer.":answered(current.userAnswer)?"Your selected answer is incorrect. The correct option is highlighted in green.":"This question was not attempted. The correct option is highlighted in green."}</p></div>
      </section>
      <aside className="solution-palette"><div className="solution-palette-head"><strong>{current.section}</strong><span>{currentIndex+1} / {review.length}</span></div><div className="solution-palette-grid">{review.map((item:any,index:number)=><button key={item.questionId||index} className={`${status(item)} ${index===currentIndex?"current":""}`} onClick={()=>setCurrentIndex(index)}>{index+1}</button>)}</div><div className="solution-legend"><span><i className="correct"/>Correct</span><span><i className="wrong"/>Incorrect</span><span><i className="empty"/>Unattempted</span></div></aside>
    </main>
    <footer className="solution-paper-footer"><button disabled={currentIndex===0} onClick={()=>setCurrentIndex((index:number)=>Math.max(0,index-1))}>← Previous</button><button className="solution-back-report" onClick={onClose}>Back to report</button><button disabled={currentIndex===review.length-1} onClick={()=>setCurrentIndex((index:number)=>Math.min(review.length-1,index+1))}>Next →</button></footer>
  </div>;
};
